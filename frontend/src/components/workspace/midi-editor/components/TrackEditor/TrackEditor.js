...
function normalizeIncomingEditorNotes(notes) {
...
const TrackEditor = ({ track, updateTrack, goBack }) => {
    const { 
    // transport
    bpm, isPlaying, playheadBeat, setPlayheadBeat, 
    ...
    useEffect(() => {
        const unsub = subscribe(async (cmd) => {
            switch (cmd.type) {
                case "SET_ZOOM":
                ...
                case "APPLY_AI_TO_TRACK": {
                    const incoming = normalizeIncomingEditorNotes(cmd.notes || []);
                    if (!incoming.length)
                        break;
    ...
    const updatePlayhead = (beat) => {
        const left = beat * gridWidth;
        if (playheadRef.current) {
            playheadRef.current.style.left = `${left}px`;
        }
    };
    const scrubToBeat = (beat) => {
        redLineRef.current = beat;
        setPlayheadBeat(beat);
        updatePlayhead(beat);
    };
    return (_jsxs("div", { style: { padding: 10, overflowY: "auto" }, children: [_jsx("div", { className: "px-4 py-3 space-y-4", children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("button", { onClick: goBack, className: "inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border hover:bg-gray-50", children: [_jsx(ChevronLeft, { className: "w-4 h-4" }), _jsx("span", { children: "Back to Tracks" })] }), _jsx("h2", { className: "text-xl font-semibold truncate", children: track.name })] }) }), _jsx(PianoRoll, { notes: track.notes, setNotes: (updated) => updateTrack({ notes: updated }), playheadRef: playheadRef, gridWidth: gridWidth, snapToGrid: snapToGrid, isPlaying: isPlaying, onPlayheadScrub: scrubToBeat })] }));
};
